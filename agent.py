...
class SimpleAgent:
    ...
    def __init__(self):
        ...
        你可以使用以下工具：​
        1. 搜索工具：当你需要获取最新、未知的信息时使用，例如'搜索 上海天气'。​
        2. 计划工具：当你需要将多个步骤整理成计划时使用，例如'制定计划 [学习Python基础，练习项目，复习算法]'。​
        3. 时间工具：当你需要知道当前时间时使用，指令就是'获取时间'。​
        4. 计算工具：当你需要进行数学计算时使用，例如'计算 (8+2)*3'。​
        """
    ...
    def _use_tool(self, tool_name, instruction):
        ...
        if tool_name == "搜索工具":
            ...
            expr = instruction
            expr = expr.replace("计算", "")
            expr = expr.replace("一下", "")
            expr = expr.replace("结果", "")
            expr = expr.strip()
...
if __name__ == "__main__":
    print("🛠️启动 SimpleAgent...")
    my_agent = SimpleAgent()
   
    # 测试几个不同的任务​
    test_tasks = [
        "我想学习agent，帮我推荐一些学习资源链接",
        "此刻的钟点是？",
        "计算一下 1+5 的结果",
    ]
