...
class VectorStore:
    ...
    async def add_documents(self, documents: List[str], metadatas: Optional[List[Dict]] = None,
        ...
        self.collection.add(documents=documents, embeddings=embeddings,
                           metadatas=metadatas or [{} for _ in documents], ids=ids)
        return ids
    
    async def search(self, query: str, top_k: int = None, filter_dict: Optional[Dict] = None) -> List[Dict]:
        top_k = top_k or settings.TOP_K
        query_embedding = (await llm_client.embed([query]))[0]
        results = self.collection.query(query_embeddings=[query_embedding], n_results=top_k,
                                       where=filter_dict, include=["documents", "metadatas", "distances"])
        docs = []
