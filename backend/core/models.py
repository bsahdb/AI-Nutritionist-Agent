...
class User(Base):
    ...
    taste_preferences = relationship("TastePreference", back_populates="user", cascade="all, delete-orphan")
...
class TastePreference(Base):
    ...
    preference_name = Column(String(100), default="默认偏好")
    ...
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.now())
    user = relationship("User", back_populates="taste_preferences")
